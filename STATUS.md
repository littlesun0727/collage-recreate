...
最新续接：已按用户最终的小改完成“移除附属清单项 → 校验独立层 → 统一展开”。保留 layer_order，不采用删列表/新增 z 或自动补齐照片。139 项回归通过；新两图 OpenClaw + qwen3.8-flash 实测 174.625 秒，图层错误 0，两图各一次有效定位/预览/finish。用户明确要求实测后提交，本轮图层更新与此前未提交停止规则一并纳入提交；具体提交号以 git log -1 为准。
...
以下按历史记录保留，旧轮次中的“当前”“待授权”“尚未实测”等描述以本节和 PLAN 第 19 节为准。
